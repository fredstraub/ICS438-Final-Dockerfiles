...
# changed for docker
df_rides = pd.read_csv("data/sampleData/disneyland_rides_lat_long_w_sentiment.csv")
...
ride_map = map.save('data/ride_map.html')

ride_map_frame = html.Iframe(
    id = 'ride_map',
    srcDoc = open('data/ride_map.html', 'r').read(),
    width = '100%',
    height = '850'
)
...
# Tab 3A
ps_df = pd.read_csv('data/sampleData/SMOOTHED_POST_SENTIMENT.csv')
ps = ps_df['Sentiment']
ps_dates = ps_df['Date']

pp_df = pd.read_csv('data/sampleData/POST_FINANCE.csv')
...
# Tab 3B
cs_df = pd.read_csv('data/sampleData/SMOOTHED_COMMENT_SENTIMENT.csv')
cs = cs_df['Sentiment']
cs_dates = cs_df['Date']

cp_df = pd.read_csv('data/sampleData/COMMENT_FINANCE.csv')
cp_raw = cp_df['Price']
# Take natural log
cp = np.log(cp_raw)
cp = np.round(cp, 2)
